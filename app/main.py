...
DB = BASE / "edgepulse.db"
app = FastAPI(title="EdgePulse", version="0.1.0")
...
def health_score(data: Telemetry):
    score = 100
...
def find_alerts(data: Telemetry):
    alerts = []
    if data.temperature > 70:
        alerts.append(("warning", f"High temperature: {data.temperature:.1f} °C"))
    if data.voltage > 15 or data.voltage < 4:
        alerts.append(("critical", f"Voltage outside range: {data.voltage:.2f} V"))
    if data.current > 8:
        alerts.append(("warning", f"High current: {data.current:.2f} A"))
    if data.signal_strength < -100:
        alerts.append(("warning", f"Weak signal: {data.signal_strength} dBm"))
    return alerts
...
def list_devices():
    ...
    rows = db.execute("""
        SELECT d.*,
        (SELECT health_score FROM telemetry t
         WHERE t.device_id = d.id ORDER BY t.id DESC LIMIT 1) AS health_score
...
def add_telemetry(device_id: int, data: Telemetry):
    ...
    db.execute("""
        ...
        (device_id, timestamp, temperature, humidity, voltage, current,
         signal_strength, health_score)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    """, (device_id, now, data.temperature, data.humidity, data.voltage,
          data.current, data.signal_strength, score))
    ...
    for severity, message in find_alerts(data):
        db.execute(
            "INSERT INTO alerts(device_id,timestamp,severity,message) VALUES(?,?,?,?)",
...
def device_telemetry(device_id: int, limit: int = Query(30, ge=1, le=500)):
    db = get_db()
    rows = db.execute("""
        SELECT timestamp, temperature, humidity, voltage, current,
               signal_strength, health_score
...
def statistics():
    ...
    result = {
        ...
        "average_health": round(db.execute(
            "SELECT COALESCE(AVG(health_score),0) FROM telemetry"
        ).fetchone()[0])
    }
    db.close()
    return result
...
def export_csv():
    db = get_db()
    rows = db.execute("""
        SELECT d.name, t.timestamp, t.temperature, t.humidity,
               t.voltage, t.current, t.signal_strength, t.health_score
    ...
    writer.writerow([
        "device", "timestamp", "temperature", "humidity", "voltage",
        "current", "signal_strength", "health_score"
    ])
